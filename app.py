...
def point_updater(x1, y1, x2, y2, key, save_line, player_id):
    ...
    if x1 == x2:
        # up-down
        # return_string += '<rect width="50" height="50" x="" y="" style="fill:'
        #   +player_data.get_color(player_id, key)+';stroke-opacity:0.7"'
        up_tmp_lines = [
            get_line_string(x1 - 1, y1, x2 - 1, y2),
            get_line_string(x1, y1, x1 - 1, y1),
            get_line_string(x2, y2, x2 - 1, y2),
        ]
        for each in up_tmp_lines:
            ...
            # up
            return_string += (
                '<rect width="50" height="50" y="'
                + str((xx - 1) * 50)
                + '" x="'
                + str((yy) * 50)
                + '" style="fill:'
                + player_data.get_color(player_id, key)
                + ';stroke-opacity:0.7"/>'
            )
        down_tmp_lines = [
            get_line_string(x1 + 1, y1, x2 + 1, y2),
        ...
        # left-right
        left_tmp_lines = [
            get_line_string(x1, y1 - 1, x1, y1),
            get_line_string(x1, y1 - 1, x2, y2 - 1),
            get_line_string(x2, y2 - 1, x2, y2),
        ]
        ...
        right_tmp_lines = [
            get_line_string(x1, y1, x1, y1 + 1),
            get_line_string(x1, y1 + 1, x2, y1 + 1),
            get_line_string(x2, y2, x2, y2 + 1),
        ]
...
def start():
    ...
    if len(players) == 1:
        ...
        return render_template(
            "waiting.html", player_name=name, host_ip=sys.argv[1], port_no=sys.argv[2]
        )
    else:
        name = "Player 2:" + name
        players[1] = name
